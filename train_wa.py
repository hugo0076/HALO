...
import os
import time
from util import get_model, train_epoch, validate, update_bn, set_seed
...
def main():
    parser = argparse.ArgumentParser(description='Train HALO')
    parser.add_argument('--id_dataset', type=str, default='CIFAR10S', help='ID dataset name')
    parser.add_argument('--batch_size', type=int, default=1024, help='input batch size')
    parser.add_argument('--extra_data_fraction', type=float, default=0.7, help='fraction of extra data to use')
    parser.add_argument('--epochs', type=int, default=400, help='number of epochs to train')
    parser.add_argument('--report_freq', type=int, default=100, help='report frequency in steps')
    parser.add_argument('--max_lr', type=float, default=0.4, help='max learning rate for one cycle policy')
    parser.add_argument('--wa_decay', type=float, default=0.995, help='weight averaging decay')
    parser.add_argument('--lr', type=float, default=0.1, help='learning rate')
    parser.add_argument('--weight_decay', type=float, default=5e-4, help='weight decay')
    parser.add_argument('--momentum', type=float, default=0.9, help='momentum')
    parser.add_argument('--epsilon', type=float, default=8/255, help='perturbation size')
    parser.add_argument('--beta', type=float, default=2.5, help='weight for KLD loss')
    ...
    train_loader, val_loader, test_loader, oe_loader = get_datasets(args.id_dataset, args.batch_size, args.oe_dataset, verbose=args.verbose)

    if args.loss_type in ['STD', 'TRADES']:
        oe_loader = None

    model = get_model(args.model, args.id_dataset).to(device)
    # Create a copy of the model for moving average
    ma_model = copy.deepcopy(model)
    ma_model.to(device)
    # Calculate total steps for MA update
    update_steps = len(train_loader)
    total_steps = update_steps * args.epochs
    warmup_steps = int(0.025 * total_steps)  # 2.5% warmup
    ...
    # load standard model if using HALO
    std_model = None
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        ma_params = {'ma_model': ma_model, 'decay': args.wa_decay, 'warmup_steps': warmup_steps, 'update_steps': update_steps, 'epoch': epoch}

        train_loss, train_acc = train_epoch(model, std_model, train_loader, oe_loader, optimizer, scheduler, ma_params, device, args)

        # Update batch norm statistics for moving average model
        update_bn(model, ma_model)

        val_loss, val_accuracy = validate(ma_model, val_loader, args=args, device=device)
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(ma_model.state_dict(), f'training_runs/{args.run_name}/{args.run_name}_{args.id_dataset}_best.pth')
